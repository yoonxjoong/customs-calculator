...
/**
 * 관세, 개별소비세, 교육세, 부가세
 * 관세[과세가격*관세율]+개별소비세[(과세가격+관세-기준초과금)*기본소비세율]+교육세[(개별소비세*교육세율]+부가세[(과세가격+관세+개별소비세+교육세)*부가세율]
 */
@Component
@Slf4j
public class TaxCalculator6 extends AbstractTaxCalculator {
    ...
    public Product calculate(Product product) {
        ...
        BigDecimal vat = (originAmount.add(customs).add(basisSales).add(edu)).multiply(
                vatRatio.getValue());

        Ratio newCustomsRatio = customsRatio.withAmount(customs);
        Ratio newBasisSavlesRatio = basisSalesRatio.withAmount(basisSales);
        Ratio newEduRatio = eduRatio.withAmount(edu);
        Ratio newVatRatio = vatRatio.withAmount(vat);

        return product.withRatios(
                List.of(newCustomsRatio, newBasisSavlesRatio, newEduRatio, newVatRatio));
    }
}
